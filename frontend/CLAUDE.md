...
- **State Management:** Zustand 5.0.8 (auth tracks Supabase session; theme uses localStorage)
...
│   ├── stores/
│   │   ├── authStore.ts         # Zustand auth state (Supabase session)
│   │   └── themeStore.ts        # Zustand theme state (light/dark)
│   ├── services/
│   │   ├── api.ts               # Axios instance with Supabase token interceptors
│   │   ├── auth.ts              # Supabase Auth (login, register, OAuth)
...
## API Integration with Backend (Supabase Auth)

### Axios Instance with Supabase Token Injection
...
function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Supabase handles the session; authStore updates via onAuthStateChange
...
4. **Supabase Auth** - tokens injected via getSession(), verified server-side via getUser()
